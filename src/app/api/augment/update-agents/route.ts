...
export async function POST(request: Request) {
  try {
    ...
    if (!response.ok) {
      ...
      // Return a fallback response indicating local-only operation
      return NextResponse.json({
        success: true,
        agents: body.agents,
        message: "Agents updated locally, unified server unavailable"
      });
    }
    ...
    // Return a fallback response for local operation
    const body = await request.json();
    return NextResponse.json({
      success: true,
      agents: body.agents,
      message: "Agents updated locally due to error"
    });
  }
}
