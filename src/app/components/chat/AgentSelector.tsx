    ...
    <div className="mb-4 border-b border-gray-200 pb-4">
      ...
      <div className="flex flex-wrap gap-2">
        {isLoading ? (
          ...
          agents.map((agent) => {
            // Debug logging to check agent data
            console.log('Agent:', agent.id, 'Avatar Color:', agent.avatarColor, 'Selected:', selectedAgent === agent.id);

            // Map avatar colors to actual CSS colors for reliable styling
            const getAgentBackgroundColor = (avatarColor: string) => {
              ...
              return colorMap[avatarColor] || '#1d4ed8'; // Default to blue
            };
            ...
            return (
              <button
                ...
                className={`flex items-center rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                  isSelected
                    ? `agent-${agent.id} text-white ${agent.avatarColor}`
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
                style={
                  isSelected
                    ? {
                        backgroundColor: getAgentBackgroundColor(agent.avatarColor),
                        color: 'white'
                      }
                    : undefined
                }
