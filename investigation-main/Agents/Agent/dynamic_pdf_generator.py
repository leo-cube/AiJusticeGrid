...
Use bullet points with bold headers where appropriate. Be thorough and professional.""",
    ...
    def generate_investigation_pdf(self, data: Dict[str, Any], analysis_type: str = "general") -> BytesIO:
        """
        Generate a PDF report in the exact format of the sample case analysis.
        Contains only CASE DETAILS and ANALYSIS sections.
        ...
        # Create the PDF document with minimal margins for more content
        doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=50, leftMargin=50,
                               topMargin=50, bottomMargin=50)

        # Get custom styles
        styles = self.create_investigation_styles()
        ...
        case_details = self.format_case_details(data)
        for detail in case_details:
            story.append(Paragraph(detail, styles['case_detail']))
            ...
            for line in analysis_lines:
                ...
                if line:
                    if line.startswith('**') and line.endswith('**'):
                        # Bold headers
                        story.append(Paragraph(line, styles['analysis_header']))
                    elif line.startswith('- **') or line.startswith('  - **'):
                        # Bullet points with bold
                        story.append(Paragraph(line, styles['bullet_point']))
                    elif line.startswith('-') or line.startswith('  -'):
                        # Regular bullet points
                        story.append(Paragraph(line, styles['bullet_point']))
                    else:
                        # Regular text
                        story.append(Paragraph(line, styles['analysis_text']))
                else:
    ...
    def format_case_details(self, data: Dict[str, Any]) -> List[str]:
        """Format case details in the exact format of the sample."""
        details = []
